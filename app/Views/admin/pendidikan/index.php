...
<div class="card mb-3">
    ...
    <div class="card-body">
        <div class="row">
            <div class="col-lg-8">
                <h3 class="mb-0">List Pendidikan Terakhir</h3>
            </div>
        </div>
    </div>
</div>
...
<div class="card mb-3">
    <div class="card-header">
        <div class="row flex-between-center">
            <div class="col">
                <button class="btn btn-primary" onclick="adddata()"><i class="fas fa-plus-square"></i> Pendidikan Terakhir</button>
            </div>
        </div>
    </div>
...
<script>
    function dataindex() {
        $('#table_index').DataTable({
            ...
            'ajax': {
                'url': '<?= site_url('admin2045/pendidikan/loaddata') ?>',
                'headers': {
    ...
    function deletedata(iddata) {
        ...
        $("#click_yes").off("click").on("click", function() {
            $.ajax({
                type: 'DELETE',
                url: "<?= site_url('admin2045/pendidikan/delete') ?>/" + iddata,
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
    ...
    function editdata(iddata) {
        $.get("<?= site_url('admin2045/pendidikan/edit') ?>/" + iddata, function(data, status) {
    ...
    function adddata() {
        $('#editor_add').load('<?= site_url('admin2045/pendidikan/add') ?>', function() {
            $('#add').modal({
                show: true
            });
        });
    ...
    function detaildata(iddata) {
        $.get("<?= site_url('admin2045/pendidikan/detail') ?>/" + iddata, function(data, status) {
