...
<div class="card mb-3">
    <div class="card-header">
        <div class="row flex-between-center">
            <div class="col">
                <button class="btn btn-primary" onclick="adddata()"><i class="fas fa-plus-square"></i> Pekerjaan</button>
            </div>
        </div>
    </div>
...
<script>
    function dataindex() {
        $('#table_index').DataTable({
            ...
            'ajax': {
                'url': '<?= site_url('admin2045/pekerjaan/loaddata') ?>',
                'headers': {
    ...
    function deletedata(iddata) {
        ...
        $("#click_yes").off("click").on("click", function() {
            $.ajax({
                type: 'DELETE',
                url: "<?= site_url('admin2045/pekerjaan/delete') ?>/" + iddata,
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
    ...
    function editdata(iddata) {
        $.get("<?= site_url('admin2045/pekerjaan/edit') ?>/" + iddata, function(data, status) {
    ...
    function adddata() {
        $('#editor_add').load('<?= site_url('admin2045/pekerjaan/add') ?>', function() {
            $('#add').modal({
                show: true
            });
        });
    ...
    function detaildata(iddata) {
        $.get("<?= site_url('admin2045/pekerjaan/detail') ?>/" + iddata, function(data, status) {
